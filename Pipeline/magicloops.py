...
def define_clfs_params(grid_size):
    ...
    smaller_grid = { 
    'RF':{'n_estimators': [10,100], 'max_depth': [5,50], 'max_features': ['sqrt'],'min_samples_split': [2]},
    'LR': { 'penalty': ['l1'], 'C': [0.1,1,10]},
    'SGD': { 'loss': ['hinge','log','perceptron'], 'penalty': ['l2','l1','elasticnet']},
    'ET': { 'n_estimators': [10,100], 'criterion' : ['gini', 'entropy'] ,'max_depth': [5,50], 'max_features': ['sqrt','log2'],'min_samples_split': [2,10]},
    'AB': { 'algorithm': ['SAMME'], 'n_estimators': [1,10,100]},
    'GB': {'n_estimators': [10,100], 'learning_rate' : [0.1],'subsample' : [1.0], 'max_depth': [5]},
    'NB' : {},
    'DT': {'criterion': ['gini', 'entropy'], 'max_depth': [1,5,10,20,50,100], 'max_features': ['sqrt','log2'],'min_samples_split': [2,5,10]},
    'SVM' :{'C' :[1],'kernel':['rbf','linear']},
    'KNN' :{'n_neighbors': [1,5,10,25,50,100],'weights': ['uniform','distance'],'algorithm': ['auto','ball_tree','kd_tree']}
           }
    ...
    elif (grid_size == 'smaller'):
        return clfs, smaller_grid
    elif (grid_size == 'test'):
...
def clf_loop(models_to_run, clfs, grid, X, y, add_cnf=True):
    results_df =  pd.DataFrame(columns=('model_type','clf', 'parameters', 'auc-roc','p_at_5', 'p_at_10', 'p_at_20','cnf','runtime'))
    for n in range(1, 2):
...
def main():
    ...
    df = pd.read_csv("/Users/arianisfeld/MachineLearning/MachineLearning/data/credit-data.csv")
